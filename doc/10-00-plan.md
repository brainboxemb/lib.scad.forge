...
Migration-011 repository baseline issue #32 is complete and qualified on exact
main, including published Build/Verification provenance on the accepted
`tool.git-project v0.2.14` / `tool.scad-project v0.15.11` stack.

The current Forge-local owner step is issue #30: retire the obsolete
`fg_overlap_mm()` compatibility alias before the next pre-1.0 release. The
canonical replacement is `FG_OVERLAP_MM()`.
