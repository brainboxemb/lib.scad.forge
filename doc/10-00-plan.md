...
Forge's Migration-011 repository baseline (#32) is complete and qualified on
exact main. The current owner step is issue #30: retire the obsolete
`fg_overlap_mm()` compatibility alias before the next pre-1.0 release.

After #30 is qualified on exact main, the resulting owner state becomes the
