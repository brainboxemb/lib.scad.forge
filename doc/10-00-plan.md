...
Forge owner work is intentionally split:

1. issue #32 aligns this repository with the accepted tooling/workflow/managed-
   launcher/documentation baseline;
2. issue #30 retires the obsolete `fg_overlap_mm()` compatibility alias;
3. the resulting owner state is released as the next pre-1.0 Forge baseline.
...
1. complete and qualify Migration-011 repository baseline issue #32;
2. retire `fg_overlap_mm()` through issue #30;
3. release v0.4.0 with `fg_res_scope()` canonical and `fg_res_apply()`
   deprecated;
4. migrate known BrainboxEmb `fg_res_apply()` consumers;
5. remove `fg_res_apply()` in the following pre-1.0 minor release.
